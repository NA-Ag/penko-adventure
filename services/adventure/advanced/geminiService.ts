...
import { SYSTEM_PROMPTS } from '../../../data/adventure/advanced/systemPrompts';
...
export class GameEngine {
  ...
  async initGame(): Promise<GameTurnData> {
    ...
    if (this.educationalScenario) {
        const promptSet = getEducationalPromptSet(this.profile.targetLanguage, this.profile.cefrLevel || 'A2') as any;
        const localScenario = promptSet.LOCALIZED_SCENARIOS?.[this.educationalScenario.id] || this.educationalScenario;

        systemInstruction = `You are an AI Language Tutor for ${this.profile.targetLanguage} at CEFR Level ${this.profile.cefrLevel || 'A2'}.
    ...
    2. The 'narrative' field MUST be entirely in ${this.profile.targetLanguage}. NO ENGLISH.
    3. Keep the 'narrative' to exactly 1 or 2 sentences, appropriate for ${this.profile.cefrLevel || 'A2'} learners.
    4. Advance the scenario dynamically based on the user's action.`;
    }
