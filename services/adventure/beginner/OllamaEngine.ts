...
export class OllamaEngine {
    ...
    private buildSystemPrompt(): string {
        return `You are a friendly text adventure game master for beginners. 
...
- Vocabulary: CEFR A1 level only.
- ROMANIZATION: If ${this.profile.targetLanguage} is non-Latin, add [ROMANIZATION: phonetic sounds] at the end.
Genre: ${this.profile.theme}.

You must return ONLY a raw JSON object. Use this exact format:
{
  "narrative": "(One short sentence in ${this.profile.targetLanguage})",
  "romanization": "(Phonetic sounds if needed)",
  "biome": "(forest, cave, town, desert, dungeon, interior, graveyard, cyber_city, or canyon)",
  "features": ["(item1)"],
  "entities": [],
  "time": "day",
  "options": ["Simple Option 1"]
}`;
    }
