...
function processLanguage(langCode, levelName, isBeginner) {
    ...
    for (const line of lines) {
        if (!foundSystem && !capturing && line.includes('<|im_start|>system')) {
            foundSystem = true;
            continue;
        }
        if (foundSystem) {
            // First line after system is the Penko intro line. Skip it and start capturing next line.
            foundSystem = false;
            capturing = true;
            continue;
        }
        if (capturing) {
            // Stop capturing on Example block or end of prompt
...
allLevels.forEach(level => {
    ...
    console.log(`Successfully bullet-proof rebuilt ${level} prompts.`);
});
