 ...
 *
 * Usage: deno run --allow-read --allow-write scripts/convert-facade-data.ts
 *        or: node scripts/convert-facade-data.ts
 */

import * as fs from 'fs';
import * as path from 'path';

// Manual type definitions (avoid import issues during conversion)
enum BeatID {
  NOTABEAT = -1,
}
enum DiscourseActType {
  NULL = -1,
  Agree = 0,
  Disagree = 1,
  Praise = 11,
...
type FacadeBeat = any;
type FacadeDataBundle = any;
type FacadeInitialWorldState = any;
type DiscourseActPattern = any;
 ...
 */
function readJavaFile(relativePath: string): string {
  const fullPath = path.join(FACADE_SOURCE_DIR, relativePath);
  return fs.readFileSync(fullPath, 'utf-8');
}

/**
 * Extract static final int constants from a Java interface/class
 */
function extractEnumConstants(javaContent: string): Record<string, number> {
 ...
 */
function extractMethodCalls(javaContent: string, methodPattern: string): string[] {
  const calls: string[] = [];
...
/**
 * Parse beat action method to extract logic
 */
function parseBeatActionMethod(methodName: string, javaContent: string): any {
  // Find the method
...
/**
 * Extract beat definitions from BeatActions.java and BeatID.java
 */
function extractBeats(): FacadeBeat[] {
  ...
  for (const [beatName, beatId] of Object.entries(beatIDs)) {
    ...
    if (beatName.includes('ArtistAdv')) topic = 59; // artistAdv
    else if (beatName.includes('Facade')) topic = 60; // facade
    else if (beatName.includes('RockyMarriage')) topic = 61; // rockyMarriage

    // Determine affinity
    let affinity: 'grace' | 'trip' | 'neutral' = 'neutral';
    ...
    // Build beat definition
    const beat: FacadeBeat = {
      id: beatId as BeatID,
      name: beatName,
      tier,
      topic,
      ...
      preconditions: [],  // TODO: Extract from ABL files
      priority: 50,        // Default priority
      maxActivations: 1,   // Default: once per session
      ...
      steps: [],           // TODO: Extract from ABL behavior files
...
/**
 * Extract initial world state from WME definitions
 */
function extractInitialState(): FacadeInitialWorldState {
  console.log('[Phase 2] Extracting initial world state...');

  // Default initial state (from Facade game knowledge)
  const initialState: FacadeInitialWorldState = {
    globalFacts: {
      playerInvited: true,
    ...
    characterStates: {
      grace: {
        character: 'grace',
        mood: -20,              // Slightly stressed
        tension: 40,            // Moderate tension
        affinityToPlayer: 0,    // Neutral toward player initially
        affinityToPartner: -10, // Slightly negative toward Trip
        activeGoals: ['maintain_facade', 'avoid_conflict'],
        desireToReveal: 20,
        desireToMaintainFacade: 80,
        revealedSecrets: [],
        playerActions: [],
        position: 'bedroom',    // Starts in bedroom
      },

      trip: {
        character: 'trip',
        mood: -15,              // Slightly stressed
        tension: 45,            // Moderate tension
        affinityToPlayer: 0,    // Neutral toward player initially
        affinityToPartner: -10, // Slightly negative toward Grace
        activeGoals: ['maintain_facade', 'be_good_host'],
        desireToReveal: 15,
        desireToMaintainFacade: 85,
        revealedSecrets: [],
        playerActions: [],
        position: 'entryway',   // Starts at door
      },
    },
    ...
    relationshipTension: 60,   // Overall tension between Grace & Trip
    currentTier: 1,
    beatHistory: [],
...
/**
 * Create discourse act recognition patterns
 */
function createDiscourseActPatterns(): DiscourseActPattern[] {
  console.log('[Phase 3] Creating discourse act patterns...');

  const patterns: DiscourseActPattern[] = [
    // Agreement
    {
...
async function convertFacadeData() {
  ...
  // Build data bundle
  const dataBundle: FacadeDataBundle = {
    version: '1.0.0',
    beats,
    initialState,
    discourseActPatterns,
    characterBehaviors: [],  // TODO: Extract from ABL files

    totalBeats: beats.length,
    totalDiscourseActTypes: Object.keys(DiscourseActType).length / 2, // Enums have reverse mapping
