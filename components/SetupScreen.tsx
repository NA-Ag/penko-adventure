...
import EducationalSetupScreen from './setup/EducationalSetupScreen';
...
const SetupScreen: React.FC<SetupScreenProps> = ({ onStart, onContinue, initialNativeLanguage, onNativeLanguageChange }) => {
  ...
  const [setupPhase, setSetupPhase] = useState<'path' | 'proficiency' | 'settings'>('path');
  ...
  const handleStart = async () => {
      ...
      onStart(finalProfile, mode, mode === 'cloud' ? apiKey : null, payload, defaultCartridge);
  };
             ...
             <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {themes.map((theme) => (
                    <button
                        ...
                        className={`p-6 capitalize font-pixel text-2xl transition-all duration-300 shadow-[4px_4px_0_rgba(0,0,0,0.8)] active:translate-y-1 active:shadow-none border-4
